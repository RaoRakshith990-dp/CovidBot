...
public class MultipleChoiceDialogFragment2 extends DialogFragment {
    ...
    public Dialog onCreateDialog(@Nullable Bundle savedInstanceState) {
        ...
        final String[] list=getActivity().getResources().getStringArray(R.array.intensity);
        builder.setTitle("Intensity of problem").setMultiChoiceItems(list, null, new DialogInterface.OnMultiChoiceClickListener() {
            @Override
            public void onClick(DialogInterface dialog, int which, boolean isChecked) {
                if(isChecked){
                    String id=mRef.push().getKey();
                    String id1=bRef.push().getKey();
                    mRef.child("bookeddetails").child(mAuth.getCurrentUser().getUid()).child(counts).child("intensity").child(id).setValue(list[which]);
                    bRef.child("bookeddetails").child(counts).child("intensity").child(id1).setValue(list[which]);
